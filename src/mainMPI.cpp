...
int main(int argc, char **argv) {
    ...
    for (int clusterSize = 2; clusterSize < 40; clusterSize++)
    {
        ...
        // Divide the sampleCount among processes
        int localSampleCount = sampleCount / size;
        int remainder = sampleCount % size;

        // Adjust for processes with an extra iteration
        if (rank < remainder) {
            localSampleCount++;
        }

        // Run the optimization for the local portion of the iterations
