...
def extract_plot_data(data):
    ...
    plot_data = {
        ...
        'candidate_energies': [],  # List of lists, one per N - all candidate energies
    }
    ...
    for n_result in data.get('allNResults', []):
        ...
        # Extract candidate energy data from all runs
        candidate_energies_n = []  # All candidate energies across all runs
        
        for run in all_runs:
            # All candidate energies from this run
            candidates = run.get('candidates', [])
            if candidates:
                # Candidates can be a list of [cluster, energy] pairs or just energies
                for candidate in candidates:
                    if isinstance(candidate, list) and len(candidate) > 1:
                        # Format: [cluster_data, energy]
                        candidate_energies_n.append(candidate[1])
                    elif isinstance(candidate, (int, float)):
                        # Format: just energy value
                        candidate_energies_n.append(candidate)
        
        plot_data['candidate_energies'].append(candidate_energies_n)
...
def create_energy_boxplot(ax, plot_data, exact_solutions):
    """Create boxplots for candidate energies (one per N)."""
    n_values = plot_data['n_values']
    candidate_energies = plot_data['candidate_energies']
    
    # Prepare data for boxplot - one box per N
    positions = list(range(len(n_values)))
    boxplot_data = candidate_energies
    ...
    bp = ax.boxplot(boxplot_data, positions=positions, widths=0.5,
    ...
    # Color all boxes with the same color
    ...
    for i, n in enumerate(n_values):
        if n in exact_solutions:
            ...
            # Draw a line across the width of the boxplot
            box_width = 0.5
    ...
    ax.set_title('Candidate Energy Distribution', 
    ...
    candidate_patch = mpatches.Patch(color='#1f77b4', alpha=0.7, label='Candidate Energies')
    exact_line = mpatches.Patch(color='red', alpha=0.7, linestyle='--', linewidth=1, 
                                fill=False, label='Exact Solution')
    ax.legend(handles=[candidate_patch, exact_line], 
...
def main():
    ...
    print("\n=== Summary Statistics ===")
    print(f"{'N':>3} | {'Exact':>12} | {'Candidates':>36}")
    print(f"{'':>3} | {'Energy':>12} | {'Min':>12} {'Max':>12} {'Avg':>12}")
    print("-" * 80)
    
    for i, n in enumerate(plot_data['n_values']):
        cand_energies = plot_data['candidate_energies'][i]
        exact_energy = exact_solutions.get(n, float('nan'))
        
        if cand_energies:
            cand_min = min(cand_energies)
            cand_max = max(cand_energies)
            cand_avg = np.mean(cand_energies)
            
            print(f"{n:3d} | {exact_energy:12.6f} | "
                  f"{cand_min:12.6f} {cand_max:12.6f} {cand_avg:12.6f}")
