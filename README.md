...
=========
 ...
 - (FUTURE CHANGE) set discreteCluster cell size to something a bit smaller than the cutoff distance (say 1.9, instead of 2.1). This allows cells to be limited to 16 atoms (estimate of max atoms in FCC for 1.9 is somewhere around 11-12), saving potentially half our computations (only losing a small bit of accuracy in the rare case that an atom 2.1 away from the central atom is 2 cells away from the one the central atom occupies).
